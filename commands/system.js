...
cmd({
        ...
        async(Void, citel, text,{ isCreator }) => {
            ...
            if (!text) return citel.reply("🔍 ᴘʟᴇᴀꜱᴇ ᴘʀᴏᴠɪᴅᴇ ᴍᴇ ᴀ ɢᴇᴛ ᴠᴀɪʟᴅ ɢɪꜱᴛ ᴜʀʟ-ᴅᴀʀᴋ ꜱʜᴀᴅᴏᴡ ᴍᴅ ⛔.")
            await addnote(text)
            return citel.reply(`ɴᴇᴡ ɴᴏᴛᴇ ${text} ᴀᴅᴅᴇᴅ ɪɴ ᴍᴏɴɢᴏᴅᴘ✬.`)
...
cmd({
        ...
        async(Void, citel, text) => {
            ...
            await sleep(20 * 1000)
            return citel.reply('ʏᴏᴜʀ ꜱᴇꜱꜱɪᴏɴ ɪꜱ ᴏᴠᴇʀ ɴᴏᴡ㉿.')
...
cmd({
    ...
    async(Void, citel, text) => {
        if (!citel.quoted) return await citel.reply(`*ʀᴇᴘʟʏ ᴛᴏ ɢᴇᴛ ᴀɴʏ ɪᴍᴀɢᴇ/ᴠɪᴅᴇᴏ ᴛᴏ ɢᴇᴛ ᴜʀʟ ✓*`)
        let mime = citel.quoted.mtype
        if(mime !='videoMessage' && mime !='imageMessage' ) return await citel.reply("▣ᴘʟᴇᴀᴄᴇ, ʀᴇᴘʟʏ ᴛᴏ ᴀɴ ɪᴍᴀɢᴇ/ᴠɪᴅᴇᴏ")
        let media = await Void.downloadAndSaveMediaMessage(citel.quoted);
        let anu = await TelegraPh(media);
        await citel.reply('*ᴛʜɪꜱ ɪꜱ ʏᴏᴜʀ ᴜʀʟ ʏᴏᴜʀ ᴍᴇᴅɪᴀ⊙.\n'+util.format(anu));
...
Void.sendMessage(citel.chat, { 
          ...
          await Void.sendMessage(citel.chat, { audio: {url : 'https://github.com/zim-bot/zimbot-v4/blob/ec8216f6e104dd3fdd143779d210c995037a0ba0/Zimbot/alive.mp3',}, mimetype: 'audio/mpeg', ptt: true }, { quoted: citel, });
            let alivemessage = Config.ALIVE_MESSAGE || `*A bot developed by Ruchira.*`
            const alivtxt = `
